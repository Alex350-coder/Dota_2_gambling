...
import { updateStreamerCommissionSchema } from "../../schemas";
...
export async function PATCH(request: Request, { params }: RouteParams): Promise<Response> {
  return runRoute({
    ...
    handler: async () => {
      ...
      const body = await parseJsonBody(request, updateStreamerCommissionSchema);
      const streamer = await container.updateStreamerCommission.execute({
        actorId: userId,
        streamerId: parsedId.data.id,
        ...body,
      });
      return NextResponse.json({ streamer }, { status: 200 });
    },
  });
}
