...
import { addMatchParticipantSchema } from "../../../schemas";
...
export async function POST(request: Request, { params }: RouteParams): Promise<Response> {
  return runRoute({
    ...
    handler: async () => {
      ...
      if (!parsedId.success) {
        throw new DomainError("VALIDATION_FAILED", "match id must be a UUID");
      }
      ...
      const body = await parseJsonBody(request, addMatchParticipantSchema);
      const participant = await container.addMatchParticipant.execute({
        actorId: userId,
