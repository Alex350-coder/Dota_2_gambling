...
import { createMarketTypeSchema } from "../schemas";
...
export async function POST(request: Request): Promise<Response> {
  return runRoute({
    ...
    handler: async () => {
      ...
      const body = await parseJsonBody(request, createMarketTypeSchema);
      const marketType = await container.createMarketType.execute({ actorId: userId, ...body });
      return NextResponse.json({ marketType }, { status: 201 });
    },
  });
}
