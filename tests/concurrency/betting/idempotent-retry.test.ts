...
/**
 * CC-07: two truly concurrent retries of the same placement with the same `Idempotency-Key`
 * must never create two orders. The loser either replays the winner's cached response or,
 * if the winner hasn't committed yet, observes `SERVICE_BUSY` (503) — both are acceptable,
 * a second `bet_orders` row is not.
 */
describe("POST /bets idempotency-key race (CC-07)", () => {
  ...
  it("never creates more than one order across truly concurrent retries of the same key", async () => {
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      const responses = await Promise.all([
        placeBetRoute(postRequest("/api/v1/bets", requestBody, cookie, idempotencyKey)),
        placeBetRoute(postRequest("/api/v1/bets", requestBody, cookie, idempotencyKey)),
      ]);
      ...
      const orderCount = await pool
        .query("SELECT count(*)::int AS n FROM bet_orders WHERE market_id = $1 AND user_id = $2", [
          marketId,
          userId,
        ])
