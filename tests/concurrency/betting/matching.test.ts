...
/**
 * CC-06: the same user placing interleaved orders across two distinct markets concurrently
 * must never deadlock — each placement acquires its own market's advisory lock (T-505) and
 * `FOR UPDATE` wallet row in a fixed, single-market-at-a-time sequence per transaction, so two
 * transactions targeting different markets never hold-and-wait on each other's lock.
 */
describe("PlaceOrderUseCase interleaved cross-market concurrency (CC-06)", () => {
  ...
  it("never deadlocks when the same user interleaves placements across two markets", async () => {
    const marketOne = await createOpenMarket();
    const marketTwo = await createOpenMarket();
    const userId = await createUser(1_000_000n);
