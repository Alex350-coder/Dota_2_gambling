...
const MIN_ORDERS_PER_SECOND = 50;

/**
 * MET-PERF-05 (`Claude/Metrics.md`): matching throughput per market must sustain
 * >= 50 orders/s. Each order here matches (fully or partially) against the same market's
 * single large counterparty, so every placement exercises the advisory-lock + FIFO scan +
 * allocation path, not just an unmatched reservation.
 */
describe("PlaceOrderUseCase matching throughput (MET-PERF-05)", () => {
  ...
  async function createUser(availableMinor = 100_000_000n): Promise<string> {
  ...
  it(`sustains >= ${MIN_ORDERS_PER_SECOND} orders/s of matched placements against a single market`, async () => {
    const { marketId, outcomeAId, outcomeBId } = await createOpenMarket();

    const counterpartyId = await createUser();
    await placeOrder.execute({
      userId: counterpartyId,
      marketId,
      outcomeId: outcomeBId,
      requestedMinor: 50_000_000n,
