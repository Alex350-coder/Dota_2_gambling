...
describe("ListBetsUseCase (T-513)", () => {
  ...
  const listBets = new ListBetsUseCase<DbTx>({
    uow,
    betOrders: (tx, ownerId) => new DrizzleOrderRepository(tx, ownerId),
  });
  ...
  async function createUser(availableMinor = 1_000_000n): Promise<string> {
  ...
  it("lists only the caller's own orders, newest first", async () => {
    ...
    const orderOne = await placeOrder.execute({
      userId: owner,
      marketId,
    ...
    const orderTwo = await placeOrder.execute({
      userId: owner,
      marketId,
      outcomeId: outcomeAId,
    ...
    const page = await listBets.execute({ actorId: owner });

    expect(page.total).toBe(2);
    expect(page.items.map((order) => order.id)).toEqual([orderTwo.id, orderOne.id]);
  });
  ...
  it("filters by status", async () => {
    ...
    const open = await placeOrder.execute({
      userId: owner,
      marketId,
      outcomeId: outcomeAId,
      requestedMinor: 1_000n,
    ...
    const toCancel = await placeOrder.execute({
      userId: owner,
      marketId,
      outcomeId: outcomeAId,
      requestedMinor: 2_000n,
    ...
    await cancelOrder.execute({ actorId: owner, orderId: toCancel.id });

    const cancelledPage = await listBets.execute({ actorId: owner, status: "CANCELLED" });
    expect(cancelledPage.items.map((order) => order.id)).toEqual([toCancel.id]);

    const openPage = await listBets.execute({ actorId: owner, status: "OPEN" });
    expect(openPage.items.map((order) => order.id)).toEqual([open.id]);
  });

  it("filters by marketId", async () => {
    const marketOne = await createOpenMarket();
    const marketTwo = await createOpenMarket();
    const owner = await createUser();

    const orderInMarketOne = await placeOrder.execute({
      userId: owner,
      marketId: marketOne.marketId,
      outcomeId: marketOne.outcomeAId,
      requestedMinor: 1_000n,
      idempotencyKey: randomUUID(),
    });
    await placeOrder.execute({
      userId: owner,
      marketId: marketTwo.marketId,
      outcomeId: marketTwo.outcomeAId,
      requestedMinor: 1_000n,
      idempotencyKey: randomUUID(),
    });

    const page = await listBets.execute({ actorId: owner, marketId: marketOne.marketId });
    expect(page.items.map((order) => order.id)).toEqual([orderInMarketOne.id]);
  });
});
