...
describe("GetBetUseCase (T-513)", () => {
  ...
  it("returns the owner's own order with its (empty) allocations", async () => {
    ...
    const result = await getBet.execute({ actorId: userId, orderId: order.id });

    expect(result.order.id).toBe(order.id);
    expect(result.allocations).toEqual([]);
  });

  it("raises RESOURCE_NOT_FOUND for another user's order (RULE-E02)", async () => {
    const { marketId, outcomeAId } = await createOpenMarket();
    ...
    await expect(getBet.execute({ actorId: attacker, orderId: order.id })).rejects.toMatchObject({
      code: "RESOURCE_NOT_FOUND",
    });
  });

  it("raises RESOURCE_NOT_FOUND for a nonexistent order id", async () => {
    const userId = await createUser();
    await expect(getBet.execute({ actorId: userId, orderId: randomUUID() })).rejects.toMatchObject({
      code: "RESOURCE_NOT_FOUND",
    });
  });
});
