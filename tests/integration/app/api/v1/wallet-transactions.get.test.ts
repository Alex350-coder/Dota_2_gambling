...
const { GET: listTransactionsRoute } = await import("@/app/api/v1/wallet/transactions/route");
...
describe("GET /wallet/transactions (T-805)", () => {
  ...
  async function createUser(availableMinor = 100_000n): Promise<string> {
    ...
    await pool.query(
      ...
      [userId, `wallet-tx-${randomUUID()}@example.test`],
    );
    await pool.query(
  ...
  it("returns 401 without a session", async () => {
    const response = await listTransactionsRoute(makeRequest("GET", "/api/v1/wallet/transactions"));
    expect(response.status).toBe(401);
  });

  it("lists only the caller's own USER_AVAILABLE ledger entries, newest first", async () => {
    ...
    await placeBet(ownerCookie, marketId, outcomeAId, "1000");
    await placeBet(ownerCookie, marketId, outcomeAId, "2000");
    await placeBet(otherCookie, marketId, outcomeAId, "3000");

    const response = await listTransactionsRoute(
      makeRequest("GET", "/api/v1/wallet/transactions", { cookie: ownerCookie }),
    );
    expect(response.status).toBe(200);
    const body = (await response.json()) as {
      transactions: { kind: string; signedAmountMinor: string; createdAt: string }[];
      meta: { total: number; page: number; limit: number };
    };
    expect(body.meta.total).toBe(2);
    expect(body.transactions).toHaveLength(2);
    expect(body.transactions.every((t) => t.kind === "RESERVE")).toBe(true);
    expect(body.transactions.map((t) => t.signedAmountMinor).sort()).toEqual(["-1000", "-2000"]);
  });

  it("paginates with a hard max page size (RULE-G04)", async () => {
    const { marketId, outcomeAId } = await createOpenMarket();
    const owner = await createUser(1_000_000n);
    const ownerCookie = await loginCookie(owner);

    for (let i = 0; i < 3; i += 1) {
      await placeBet(ownerCookie, marketId, outcomeAId, "100");
    }

    const response = await listTransactionsRoute(
      makeRequest("GET", "/api/v1/wallet/transactions?page=1&limit=2", { cookie: ownerCookie }),
    );
    expect(response.status).toBe(200);
    const body = (await response.json()) as {
  ...
  it("never returns another user's transactions (RULE-E02)", async () => {
    const { marketId, outcomeAId } = await createOpenMarket();
    const owner = await createUser();
    ...
    await placeBet(ownerCookie, marketId, outcomeAId, "1500");

    const response = await listTransactionsRoute(
      makeRequest("GET", "/api/v1/wallet/transactions", { cookie: attackerCookie }),
    );
    expect(response.status).toBe(200);
    const body = (await response.json()) as { transactions: unknown[]; meta: { total: number } };
